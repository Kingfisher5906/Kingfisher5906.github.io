...
let errors = ""
    ...
    {   
        ...
        if(debug)console.log("DEBUG:Scripts.js:query: Recieved class amount as: " + count)
        for(var i = 1; i < 8; i++) //put courses in string
        {
            courses = (courses + getQueryVariable("c" + i) + "~")
            
        }
        for(var i = 1; i < 8; i++) //put grades in string
        {
    ...
    function getQueryVariable(variable)
    {
        ...
        if(debug)console.log("ERROR:Scripts.js: Unexpected input lead to an error in paramater splitting function.")
        errors = errors + "ERROR:Scripts.js: Unexpected input lead to an error in paramater splitting function.  "
        return(false);
    }
